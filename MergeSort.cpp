...
void merge(T* array_ptr, unsigned int first_idx, unsigned int second_idx, unsigned int array_len) // params: array, start index of first array, start index of second array, and array length
{
	...
	unsigned int len_second = array_len - len_first; 
...
{
	unsigned int div_idx = std::floor(array_len / 2); // divide array into two subarrays
	if (div_idx > 1)
	{	
        	merge_sort<T>(array_ptr, div_idx, start_idx); // recursively sort the first subarray
        	merge_sort<T>(array_ptr, array_len - div_idx, start_idx + div_idx); // recursively sort the second subarray
        
        	merge<T>(array_ptr, start_idx, start_idx + div_idx, array_len); // merge the two subarrays into one sorted array
	}
}
